...
function getDiv(flightSlices, sliceReducer) {
  var html = flightSlices.reduce(function(div, flightSlice) {
    return div + sliceReducer(flightSlice) + '<br/>';
  }, '<div>');
  return html + '</div>'
}
...
function updateFlights(flights) {
  ...
  flights.options.forEach(function (option) {
    // TODO: only doing first way atm
    var flightSlice = option.flightSlices[0];
    ...
    table += '<tr class="flight">' +
      ...
      getDiv(option.flightSlices, function(slice) {
        ...
        var airline = slice.legs.reduce(function (prevAirline, flight){
          if (flight.carrier === prevAirline && prevAirline != null) {
            return 'Multiple';
          }
...
/*$('#searchButton').click(function() {
  $("#selection").show();
  $("#data2").show();
  //we are removing all rows using remove if already exists
  ...
  $.getJSON( "flights.json", function(response) {
    ...
    for (var i=0;  i<len; i++) {
      ...
      table += '<tr class="flight"><td class="flightDtls">'+source+'</td><td class="flightDtls">'+dest+'</td><td class="flightDtls">'+dept_time+'</td><td class="flightDtls">'+arrival_time+'</td><td class="flightDtls">'+duration+'</td>';
      var price = (response.options[i].totalSales).replace("USD".toUpperCase(),"$ ");
      table += '<td class="flightDtls">'+price+'</td>';
      table += '<td class="flightDtls"><input type="button" class ="gridButton" value = "select" onclick="onButtonSelect(\''+source+'\',\''+dest+'\',\''+dept_time+'\',\''+arrival_time+'\',\''+duration+'\',\''+price+'\');"></input></td></tr>';
    }
    table+="</table>";
    $('#data2').append(table);
  });
...
$(document).ready(function () {
  document.getElementById('dept').innerHTML = $('#dest').val();
});*/